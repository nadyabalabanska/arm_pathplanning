...
def retry(f, *args, **kwargs):
    while not f(*args, **kwargs):
        retry = raw_input("Move command failed. Retry? [y/n]: ")
        if retry == 'y':
            print("retrying... ")
            continue
        else:
...
def initial_canvas_home(arm):
    ...
    arm.move_to_joint_positions(manipulator_tools.JointPositions.CANVAS_HOME)
    time.sleep(.1) 
    arm.velocity_mode_precise()
    retry(arm.move_to_joint_positions, manipulator_tools.JointPositions.CANVAS_HOME)
...
def interactive_depth_config(arm):
    ...
    retry(arm.move_relative_pose, z=manipulator_tools.DOWNWARD_OFFSET,
                                  cartesian=True,
                                  cartesian_resolution=0.0025)
    time.sleep(.1)
    return arm.location()
...
def paint(arm, filename):
    ...
    for action in actions:
        ...
        if isinstance(action, painting_tools.PaintingAction.Color):
            arm.velocity_mode_fast()
            arm.move_to_location_cartesian(arm.target_locations.brush_home, resolution=0.02)
            arm.velocity_mode_precise()
            time.sleep(.1) 
            retry(arm.move_to_location_cartesian, arm.target_locations.brush_home, resolution=0.005, travel_height=arm.location()[2])
            time.sleep(.1)
            print("    - arrived at brush_home")
            print("      waiting...")
            time.sleep(3)
            ...
            arm.velocity_mode_fast()
            arm.move_to_location_cartesian(arm.target_locations.canvas_home, resolution=0.02)
            time.sleep(.1) 
            arm.velocity_mode_precise()
            retry(arm.move_to_location_cartesian, arm.target_locations.canvas_home, resolution=0.0025, travel_height=arm.location()[2])
            time.sleep(.1)
            ...
            if fraction > 0.99:
                print("  - executing plan")
                retry(arm.execute_plan, plan, wp)
                time.sleep(.1) 
                print("  - plan executed")
            else:
                print("  - [!] Fraction not high enough. Will not execute plan.")
    ...
    print("done!")
    retry(arm.move_to_location_cartesian, arm.target_locations.canvas_home)
    time.sleep(.1)
